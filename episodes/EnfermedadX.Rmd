...
En epidemiología, las distribuciones de rezagos se refieren a los *intervalos de tiempo* inherentes desde dos fechas clave durante un brote. Por ejemplo: el tiempo entre el inicio de los síntomas y el diagnóstico, el tiempo entre el inicio de los síntomas y la muerte, entre muchos otros.

Hoy el taller se centrará en dos rezagos clave conocidos como el *período de incubación* y el *intervalo serial*. Ambos son fundamentales para brindar información para la respuesta de salud pública.


El [**periodo de incubación**]{.underline} es el tiempo entre la infección y la aparición de síntomas.

El [**intervalo serial**]{.underline} es el tiempo entre la aparición de síntomas en un par de casos primario y secundario.

La relación entre estos parámetros tiene un impacto en si la enfermedad se transmite antes  [(**Transmisión pre-sintomática)**]{.underline} o después [**(Transmisión sintomática)**]{.underline} de que el caso primario haya desarrollado síntomas (Figura 1).
...
#### 3.2. Sesgos potenciales y ajustes de los rezagos epidemiológicos

##### **2.2.1 Sesgos potenciales**

Al estimar los rezagos epidemiológicos, es importante considerar posibles sesgos:

[**Censura**]{.underline} significa que sabemos que un evento ocurrió, pero no sabemos exactamente cuándo ocurrió. La mayoría de los datos epidemiológicos están "doblemente censurados" porque hay incertidumbre en torno a ambos tiempos, tanto del tiempo del evento primario como secundario. Ignorar la censura puede llevar a estimaciones sesgadas de la desviación estándar del rezago (Park et al. en progreso).

[**Truncamiento a la derecha**]{.underline} es un tipo de sesgo de muestreo relacionado al proceso de recolección de datos. Este sesgo surge porque solo se pueden observar casos que han sido reportados. Ignorar el truncamiento a la derecha durante la fase de crecimiento de una epidemia puede llevar a una subestimación del rezagos medio (Park et al. en progreso).

*Sesgo* [**dinámico (o de fase epidémica)**]{.underline} es otro tipo de sesgo de muestreo. Este sesgo afecta a los datos retrospectivos y está relacionado con la fase de la epidemia: durante la fase de crecimiento exponencial, los casos que desarrollaron síntomas recientemente están sobrerrepresentados en los datos observados, mientras que durante la fase de declive, estos casos están subrepresentados, lo que lleva a la estimación de intervalos de rezago más cortos y más largos, respectivamente (Park et al. en progreso).
