...
| Model Option | Logarithmic Scale | Probability of positive case at age $a$ | Prior distribution |
|---|---|---|---|
| `constant` | FALSE | $\sim binom(n(a,t), P(a,t))$ | $\lambda\sim uniform(0,2)$ |
| `tv_normal` | FALSE | $\sim binom(n(a,t), P(a,t))$ | $\lambda\sim normal(\lambda(t-1),\sigma)\\ \lambda(t=1)\sim normal(0,1)$ |
| `tv_normal_log` | TRUE | $\sim binom(n(a,t), P(a,t))$ | $\lambda\sim normal(log(\lambda(t-1)),\sigma)\\ \lambda(t=1)\sim normal(-6,4)$ |
...
m2_chik <- fit_seromodel(serosurvey = chik2015,
                         model_type = "tv_normal",
                         iter = 1500)

m3_chik <- fit_seromodel(serosurvey = chik2015,
                         model_type = "tv_normal_log",
                         iter = 1500)
```
