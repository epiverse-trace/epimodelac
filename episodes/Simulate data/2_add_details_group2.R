...
sims <- readRDS("Data/mpox_sims_2nov2023.RDS")

linelist <- sims$linelist
contacts <- sims$contacts

# Pick arbitrary date to start the epidemic
dates <- data.frame(date = seq(from = as.Date("2023-05-01"), to = as.Date("2023-06-30"), by = "days"),
                    date_onset = 0:60)
...
# Want a bimodal distribution, kids and elderly
age <- round(rbinorm(n = nrow(linelist), mean1 = 4, mean2 = 70, sd1 = 2, sd2 = 5, prop = 0.5), digits = 0)
hist(age)
sort(age)
summary(age)
...
# Add some missing
exposure <- sample(x = c("Breathing",NA), size = nrow(linelist), replace = T, prob = c(0.7, 0.3))
...
# Sample IP 
#curve(dweibull(x, shape=2, scale = 8), from=0, to=20)

IP_v <- vector()
for (i in 1:nrow(IP)){
  IP_samp <- round(rweibull(n = 1, shape = 2, scale = 8), digits = 0)
  IP_v[i] <- IP_samp
}
...
saveRDS(list(linelist = linelist_new,
             contacts = contacts4), "practical_data_group2.RDS")
